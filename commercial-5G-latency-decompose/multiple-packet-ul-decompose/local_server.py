...
PHONE_LISTEN_PORT = 5001        # Port for sending data to phone client
...
phone_client_socket = None      # Socket for connected phone client
send_thread = None              # Thread for sending data to phone client
...
def handle_phone_client(client_sock, client_address):
    """Handle communication with a phone client and send data packets"""
    global phone_client_socket, running, num_requests, bytes_per_request
    
    try:
        # Store the client socket globally
        phone_client_socket = client_sock
        
        print(f"New phone client connection from {client_address}")
        
        # Send data directly in this thread
        requests_sent = 0
        ...
        time.sleep(1)
        # Reset global socket
        if phone_client_socket == client_sock:
            phone_client_socket = None
...
def listen_for_phone_clients():
    """Listen for phone client connections"""
    ...
    phone_server_socket.bind((PHONE_LISTEN_IP, PHONE_LISTEN_PORT))
    phone_server_socket.listen(1)  # Only allow one connection
    
    print(f"Listening for phone clients on TCP {PHONE_LISTEN_IP}:{PHONE_LISTEN_PORT}")
    
    try:
        while running:
            # Accept new connection
            client_sock, client_address = phone_server_socket.accept()
...
def main():
    ...
    try:
        ...
        print(f"Listening for phone clients on port {PHONE_LISTEN_PORT}")
