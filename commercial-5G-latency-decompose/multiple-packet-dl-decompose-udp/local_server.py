    ...
    try:
        ...
        while running:
            try:
                ...
                data, addr = client_socket.recvfrom(1024)  # Now expect UDP packets
                end_recv = time.time()
                if not data:
                    print("Empty data from phone")
                    continue
                
                recv_delay = (end_recv - start_recv) * 1000
                print(f"Received data from {addr}, length: {len(data)}, recv delay: {recv_delay:.3f} ms")
...
def listen_for_phone():
    ...
    # Create UDP socket for phone communication
    ...
    # Set buffer sizes to reduce latency
    try:
        # Use smaller UDP buffer (8KB)
        phone_socket.setsockopt(socket.SOL_SOCKET, socket.SO_RCVBUF, 8192)
        cur_rcvbuf = phone_socket.getsockopt(socket.SOL_SOCKET, socket.SO_RCVBUF)
        print(f"UDP receive buffer size set to: {cur_rcvbuf} bytes")
    except Exception as e:
    ...
    print(f"Listening for UDP packets from phone on {PHONE_LISTEN_IP}:{PHONE_LISTEN_PORT}")
    
    # Start a single thread to handle all client communications
    # Since UDP is connectionless, we don't need separate threads per client
    client_thread = threading.Thread(
        target=handle_phone_client,
        args=(phone_socket, ("UDP", PHONE_LISTEN_PORT))
    )
