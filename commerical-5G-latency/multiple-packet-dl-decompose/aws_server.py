    ...
    try:
        ...
        while requests_sent < num_requests:
            ...
            if bytes_per_request > 0:
                payload = os.urandom(bytes_per_request)
            ...
            combined_message = header + payload
            
            # Send combined message in a single call
            client_socket.sendall(combined_message)
            
            requests_sent += 1
            print(f"Sent request {requests_sent}/{num_requests} to {client_address}: {bytes_per_request} bytes of payload")
