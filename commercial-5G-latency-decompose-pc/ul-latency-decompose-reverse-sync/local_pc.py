...
lock = threading.Lock()         # Lock for thread-safe updates to sync data
    ...
    try:
        ...
        while running and requests_sent < num_requests:
            try:
                ...
                # Get current time offset and sync RTT
                with lock:
                    current_offset = time_offset
                    current_rtt = current_sync_rtt
                
                # Create header with request_id, timestamp, size, offset, and sync RTT
                # Format: !IdIdd = 4-byte int + 8-byte double + 4-byte int + 8-byte double + 8-byte double = 32 bytes total
                header = struct.pack('!IdIdd', request_id, timestamp, bytes_per_request, current_offset, current_rtt)
                ...
                print(f"Sent header to cloud server - Request ID: {request_id}, Timestamp: {timestamp:.6f}, "
                      f"Offset: {current_offset:.6f}s, Sync RTT: {current_rtt*1000:.2f}ms")
...
def main():
    ...
    try:
        ...
        print(f"Local client running, initiating time sync requests to cloud server")
